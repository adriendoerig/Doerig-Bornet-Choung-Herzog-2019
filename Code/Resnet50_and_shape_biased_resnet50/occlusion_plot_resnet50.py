...
for i, stim_name in enumerate(stim_names):
    ...
    for model_type in model_types:
        ...
        model_combined_maps = []
        map_name = os.path.join(occlumap_dir, '___occlu_' + str(stim_name) + '_' + resnet50s[model_type] + '_' + str(dilation) + '_combined')
        ...
        for model_ID in range(5):
            model_occlumaps = []
            these_files = [f for f in occlumap_files if (stim_name in f and f[-5] == str(model_ID) and 'combined' not in f and 'npy' in f and resnet50s[model_type] in f)]
